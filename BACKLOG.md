...
### 33. Documenter la consommation de tokens du pipeline ✅ Fait le 09/09/2026 (documentation)
**Constat** (demande explicite de l'utilisateur du 09/09/2026) : le `README.md` justifiait en détail le choix de modèle et de niveau d'effort, en mentionnant plusieurs fois le coût en tokens comme argument (Haiku « économiserait des tokens », `xhigh` par défaut « consommerait des tokens sans bénéfice mesurable », `max` « ferait dépenser des tokens sans lever la vraie contrainte »), mais ne disait nulle part **ce que le pipeline consomme réellement**, ni quelles étapes pèsent le plus, ni quoi faire pour limiter ce coût. Un consultant qui lance `formation-pipeline` sur une formation multi-jours n'avait aucun repère pour anticiper l'ordre de grandeur.
**Valeur** : permet d'anticiper le coût avant de lancer une formation complète, de savoir quelle étape pèse le plus (l'audit `comite-qualite` en dossier complet, pas la production de contenu comme on pourrait le supposer), et d'utiliser sciemment les mécanismes d'économie déjà écrits dans les specs plutôt que de les ignorer.
**Action** :
  - Nouvelle section `# Consommation de tokens` dans `README.md`, après la section modèle/effort et avant la mise en place technique.
  - Trois volets : ce qui fait varier la consommation ; une estimation par skill en ordres de grandeur ; les leviers d'économie déjà présents dans les specs ; plus une partie sur la gestion des sessions longues.
  - **Méthode d'estimation explicitée** et assumée comme non mesurée : volume de contenu final dérivé des repères chiffrés déjà écrits dans les `SKILL.md` (ex. « ~10-15 slides par heure de théorie »), converti en tokens (~1,3-1,5 token/mot en français), multiplié par un facteur de coût agentique (x3 à x8) présenté comme une plage courante, **pas** comme une mesure propre à ce dépôt.
**Effort** : S — documentation seule, aucune spec de skill modifiée.
**Dépend de** : rien.
**Statut** : section écrite dans `README.md`. **Reste à confronter à un run réel** — c'est une estimation dérivée de la forme des specs, jamais mesurée ; le premier retour d'usage chiffré (une formation complète réellement produite) doit servir à corriger ces fourchettes, qui sont volontairement larges.
...
### 29. Créer le pipeline "Réponse à appel d'offres" — refonte en skill unique bout-en-bout 🟡 Refondu le 18/08/2026 (US-18 livrée)
**Constat** (demande explicite de l'utilisateur du 29/07/2026, suite à la résolution de #17) : même besoin de production assistée par IA que le pipeline formation, mais pour la réponse aux appels d'offres commerciaux — de l'analyse du dossier AO jusqu'au plan de présentation optimisé pour Claude Design. Discovery initiale menée façon PM sénior : 4 douleurs retenues (exigence CCTP ratée/mal traitée, manque de différenciation, délai trop court, incohérence entre contributeurs), format d'entrée hétérogène (marché public structuré vs privé informel), aucun gabarit interne existant.
**Valeur** : élargit le pipeline de skills à l'avant-vente, sécurise le risque n°1 identifié (exigence obligatoire ratée = disqualification), gagne du temps sur un processus à deadline dure, va désormais jusqu'à l'analyse de fit cabinet/client et la sélection de références différenciantes.
**Révision du 18/08/2026** : après une première itération (`cadrage-appel-offres`, limitée à l'analyse du dossier), l'utilisateur a redéfini le périmètre en un workflow explicite à 8 étapes bien plus large (recherche méthodologique sur les bonnes pratiques d'AO, recherche client, analyse du besoin, analyse de fit cabinet via un profil cabinet réutilisable, sourcing et sélection de références, plan de présentation détaillé pour Claude Design, renvoi vers comité qualité). Décision actée : remplacer directement `cadrage-appel-offres` par une skill unique `reponse-appel-offres`, plutôt que de poursuivre la trajectoire initialement prévue en 4 skills séquentielles (`memoire-technique-builder`, `memoire-content-claude-design`, `appel-offres-pipeline`) — élargissement direct du périmètre, pas un abandon du principe itératif de départ.
