...
- **Actuellement** : la personne à l'origine des skills historiques du pipeline formation (`cadrage-formation`, `formation-material-builder`, `slide-content-claude-design`, `comite-qualite`, `formation-pipeline`), qui anime les deux pipelines — ce périmètre inclut désormais aussi le pipeline réponse à appel d'offres (`reponse-appel-offres` et ses évolutions futures) et les skills transverses (`design-system-extractor`, qui sert les deux pipelines ; `consultants-references-extractor`, qui alimente le pipeline AO), même Product Owner sauf mention contraire.
