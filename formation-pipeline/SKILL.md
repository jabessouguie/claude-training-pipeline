...
| `00-brief.md`/`00-plan.md` produits, pas encore validés par l'utilisateur | **Stop, y compris en mode non-stop** — attendre la validation explicite prescrite par la Phase 0.2 de `formation-material-builder` (garde-fou dur, l'orchestrateur ne peut jamais le lever, voir « Gestion des points de validation » ci-dessous — ne pas confondre avec la règle de relance implicite de `comite-qualite` Phase C, qui ne s'applique qu'aux corrections 🟠/🟡, jamais à ce type de validation structurante) |
